...
// This object maps the 'type' string from the database to the correct icon component
const iconMap = {
  Address: MapPinIcon,
  Phone: PhoneIcon,
  Email: EnvelopeIcon,
...
const Footer = () => {
  ...
  useEffect(() => {
    const fetchFooterData = async () => {
      try {
        const response = await apiClient.get("/footer-contact-details");
  ...
  return (
    <footer className={styles.footer}>
      <div className={`container ${styles.footerContainer}`}>
        ...
        {/* Column 4: Contact Info (NOW DYNAMIC) */}
        <div className={styles.footerColumn}>
          <h3 className={styles.footerHeading}>Contact Us</h3>
          <div className={styles.contactInfo}>
            {loading ? (
              ...
              contactDetails.map((detail) => {
                // Dynamically select the Icon component based on the 'type' from the database
                const IconComponent = iconMap[detail.type] || MapPinIcon; // Default to MapPinIcon if type is unknown
                return (
                  <div className={styles.contactItem} key={detail.id}>
                    <div className={styles.iconWrapper}>
                      <IconComponent />
                    </div>
                    {/* Use pre-line to respect newlines from the admin panel's text area */}
