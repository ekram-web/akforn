...
// --- MAIN COMPONENT (FINAL INTEGRATED VERSION) ---
const Guide = () => {
  ...
  useEffect(() => {
    const fetchGuides = async () => {
      try {
        ...
        // IMPORTANT: The `pdf_links` is a JSON string, we need to parse it
        const parsedGuides = response.data.map((guide) => ({
          ...guide,
          pdf_links: guide.pdf_links ? JSON.parse(guide.pdf_links) : [],
        }));
      ...
      <div className="container">
        ...
        <main className={styles.guidesList}>
          {guides.map((guide, index) => (
            <div className={styles.accordionItem} key={guide.id}>
              <button
                className={styles.accordionHeader}
                onClick={() => handleToggle(index)}
              >
              ...
              {openIndex === index && (
                <div className={styles.accordionContent}>
                  ...
                  {Array.isArray(guide.pdf_links) &&
                    guide.pdf_links.length > 0 && (
                      <div className={styles.pdfList}>
                        {guide.pdf_links.map((link, idx) => (
                          <a
                            href={`${storageUrl}/${link.path}`}
                            download={link.name}
                            key={idx}
                            className={styles.pdfLink}
                          >
